...
def test_a_secondary_candidate_is_only_consumed_by_one_match():
    gitleaks = [
    ...
    assert len(merged) == 2
    sources = {c.id: c.source for c in merged}
    assert sources["g1"] == "gitleaks+trufflehog"
    assert sources["g2"] == "gitleaks"
